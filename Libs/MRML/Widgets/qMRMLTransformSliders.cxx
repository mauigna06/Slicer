...
void qMRMLTransformSliders::setTypeOfRotation(RotationType _typeOfRotation)
{
    Q_D(qMRMLTransformSliders);

    if (d->TypeOfTransform == qMRMLTransformSliders::ROTATION)
    {
        if (_typeOfRotation == qMRMLTransformSliders::INTRINSIC_ANGLES)
        {
            d->LRSlider->setTypeOfRotation(qMRMLLinearTransformSlider::INTRINSIC_ANGLES);
            d->PASlider->setTypeOfRotation(qMRMLLinearTransformSlider::INTRINSIC_ANGLES);
            d->ISSlider->setTypeOfRotation(qMRMLLinearTransformSlider::INTRINSIC_ANGLES);

            d->LRSlider->setRange(-180.00, 180.00);
            d->PASlider->setRange(-90.00, 90.00);
            d->ISSlider->setRange(-180.00, 180.00);
        }
        else if (_typeOfRotation == qMRMLTransformSliders::EXTRINSIC_ANGLES)
        {
            d->LRSlider->setTypeOfRotation(qMRMLLinearTransformSlider::EXTRINSIC_ANGLES);
            d->PASlider->setTypeOfRotation(qMRMLLinearTransformSlider::EXTRINSIC_ANGLES);
            d->ISSlider->setTypeOfRotation(qMRMLLinearTransformSlider::EXTRINSIC_ANGLES);
